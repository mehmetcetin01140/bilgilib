  ...
  return (
    ...
    <div className="recommendationArea mt-5">
             ...
             <div key={index}>
                    ...
                    <Card.Img src={`${process.env.REACT_APP_ENDPOINT}/images/${recom.Image}`} alt={recom.Title} className="recomImage"  />
                <Card.ImgOverlay >
                <Card.Body>
                  <Card.Title><span className="respTitle">{recom.Title}</span></Card.Title>
                </Card.Body>
                  </Card.ImgOverlay>
                <Card.Footer className="text-muted recomFooter"><i className="fa-solid fa-calendar-days me-2 "></i><span>{new Date(recom.Datelog.replace(/-/g, '/').slice(0,11)).toLocaleDateString()}</span></Card.Footer>
              </Card> 
              </Link>
